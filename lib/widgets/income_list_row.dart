...
class IncomeListRow extends StatelessWidget {
  ...
  const IncomeListRow({super.key, required this.item});
    ...
    return Column(
      children: [
        Container(
          ...
          padding: EdgeInsets.fromLTRB(18 * s, hasTitle ? 23 * s : 15 * s, 18 * s, hasTitle ? 21 * s : 19 * s),
          child: Row(
            ...
            children: [
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  if (hasTitle) ...[
                    Row(
                      children: [
                        Text('工资薪金', style: TextStyle(color: TaxColors.mainText, fontSize: 22.2 * s, fontWeight: FontWeight.w500, height: 1)),
                        const Spacer(),
                        Text(item.month!, style: TextStyle(color: TaxColors.mainText, fontSize: 22.0 * s, fontWeight: FontWeight.w400, height: 1)),
                        SizedBox(width: 34 * s),
                      ],
                    ),
                    SizedBox(height: 16 * s),
                    _DetailText('所得项目小类： ${item.subType}', scale: s),
              ...
              SizedBox(width: 8 * s),
              Icon(Icons.arrow_forward_ios, color: TaxColors.arrow, size: 24 * s),
            ],
          ),
        ),
